...
resumeRouter.post("/", upload.single("resume"), async (req, res) => {
  try {
    const { fullName, email, contactNumber, jobAppliedFor, state, city } = req.body
...
resumeRouter.patch("/:id", async (req, res) => {
  try {
    ...
    const updatedResume = await Resume.findByIdAndUpdate(req.params.id, { $set: updateData }, { new: true })
