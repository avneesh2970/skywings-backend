...
newsRouter.get("/", async (req, res) => {
  try {
    const { search, status, category, tags, page = 1, limit = 10, sort = "date", order = "desc" } = req.query
...
newsRouter.post("/", upload.single("image"), async (req, res) => {
  try {
    const { title, description, content, author, date, status, category } = req.body
    let tags = []
...
newsRouter.patch("/:id/category", async (req, res) => {
  try {
    const { category } = req.body

    if (!category || !["company", "client", "industry", "general"].includes(category)) {
