...
newsletterRouter.patch("/:id", async (req, res) => {
  try {
    ...
    const updatedSubscriber = await Newsletter.findByIdAndUpdate(req.params.id, { $set: updateData }, { new: true })
...
newsletterRouter.post("/bulk", async (req, res) => {
  try {
    ...
    if (action === "update") {
      ...
      const result = await Newsletter.updateMany({ _id: { $in: ids } }, { $set: updateData })
